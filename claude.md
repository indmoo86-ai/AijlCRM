...
| 维度 | 说明 | 截图命名 | 必须 |
|------|------|---------|------|
| 1. 表单填写 | 显示测试数据已输入到表单 | `场景号-form-filled.png` | 是 |
| 2. 成功提示 | 显示操作成功的提示消息 | `场景号-success.png` | 是 |
| 3. 数据库验证 | SQL查询确认数据已入库 | `场景号-db-verify.png` 或控制台输出 | 是 |
| 4. 列表显示 | 列表页面正确显示新创建的数据 | `场景号-list-data.png` | 可选 |

**说明**:
- 维度1-3为必须验证项
- 维度4（列表显示）在以下场景可省略：登录、查询、导出、状态变更、审批等非创建类操作

### 测试执行原则（不可违反）

1. **顺序执行**：每个测试场景必须完全通过后才能进行下一个场景
2. **数据依赖**：测试场景之间存在数据依赖关系
   - 登录是所有测试的前提条件
   - 产品管理是报价单/合同的前提
   - 客户管理是线索转化的前提
   - 合同是发货/收款/发票的前提
3. **数据准备**：如果测试场景缺少必要的测试数据，必须先创建
4. **单场景测试**：每次只测试一个场景，确认通过后再进行下一个
5. **完整验证**：每个场景必须满足4个验证维度，不能因为工作量大或token限制而简化

### 测试通过标准

一个测试场景只有在满足以下所有条件时才算通过：
1. 所有操作步骤成功执行（无报错）
2. 成功提示消息正确显示
3. 数据库查询确认数据已正确写入
4. 页面列表正确显示新数据
...
```
发现问题 → 记录到 TEST_ISSUES_TRACKER.md → 分析截图和数据库 → 修复代码 → 重新测试
```

**重试策略**:
- **第1-3次修复**：直接根据错误信息和截图分析修复
- **第4-6次修复**：重新查阅需求文档和测试文档，审视问题根源
- **第7-8次修复**：考虑是否需要重构相关代码
- **超过8次**：记录为已跳过，详细说明原因和尝试过的方案，继续下一个场景
...
场景1: 用户登录 ──────────────────────────────────────────────┐
                                                              │
场景4: 产品管理 ──┬──> 场景5: 报价单 ──> 场景6: 合同 ──┬──> 场景7: 发货
                  │                                      │
场景2: 线索管理 ──┼──> 场景3: 客户管理 ─────────────────┤──> 场景8: 收款
                  │                                      │
                  └──> 场景4.x: 任务管理                  └──> 场景9: 发票
                                                              │
                                                              └──> 场景10: 售后
...
- **管理员账号**：admin / 123456
- **销售账号**：sales001 / sales123

### 测试文件位置

- **测试场景清单**：`FULL_79_TEST_SCENARIOS.md`（输入数据和预期输出）
- **测试脚本**：`tests/e2e/all-scenarios-test.js`
- **测试配置**：`tests/e2e/playwright.config.js`
- **截图目录**：`test-screenshots/`
- **问题跟踪**：`TEST_ISSUES_TRACKER.md`
- **测试报告**：`TEST_RESULTS_REPORT.json`
...
// 测试数据命名规范
const testData = {
  companyName: `测试酒店_${timestamp}`,  // 使用时间戳避免重复
  contactPerson: '张经理',
  // ...
}
```
...
```bash
# 登录数据库
mysql -u root -p aijlcrm

# 验证数据是否存在
SELECT * FROM 表名 WHERE 条件 ORDER BY created_at DESC LIMIT 1;

# 常用验证SQL
SELECT * FROM products WHERE product_name LIKE '%测试%' ORDER BY created_at DESC LIMIT 5;
SELECT * FROM customers WHERE customer_name LIKE '%测试%' ORDER BY created_at DESC LIMIT 5;
SELECT * FROM leads WHERE customer_name LIKE '%测试%' ORDER BY created_at DESC LIMIT 5;
...
完整的79个测试场景定义在：`FULL_79_TEST_SCENARIOS.md`
