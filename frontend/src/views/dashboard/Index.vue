...
const renderFunnelChart = () => {
  ...
  const data = funnelData.value.length > 0 ? funnelData.value : [
    { name: '线索', value: 0 },
    { name: '客户', value: 0 },
    { name: '报价', value: 0 },
    { name: '合同', value: 0 },
...
.shortcuts-card :deep(.el-card__body) {
  padding: 15px 20px;
}
...
.followup-stats .stat-item:hover {
  transform: translateY(-2px);
}

.followup-stats .stat-item.severe {
  background: linear-gradient(135deg, #ff6b6b 0%, #ee5a24 100%);
  color: #fff;
}

.followup-stats .stat-item.normal {
  background: linear-gradient(135deg, #ffa502 0%, #ff7f50 100%);
  color: #fff;
}

.followup-stats .stat-item.total {
  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  color: #fff;
}

.followup-stats .stat-value {
  font-size: 32px;
  font-weight: bold;
  line-height: 1.2;
}
