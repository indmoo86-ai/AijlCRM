...
async function test_6_1_create_contract() {
  console.log('========================================');
  console.log('场景6.1: 创建合同测试');
  console.log('========================================');
  ...
  const contractTitle = `测试合同_${timestamp}`;
  ...
  try {
    ...
    // Step 4: 填写合同表单
    console.log('Step 4: 填写合同表单...');
    await page.waitForSelector('.el-dialog__body', { timeout: 5000 });
    await page.waitForTimeout(500);

    // 填写合同标题（必填）
    const titleInput = await page.$('.el-dialog__body input[placeholder*="合同标题"]');
    if (titleInput) {
      await titleInput.fill(contractTitle);
      console.log('  合同标题:', contractTitle);
    }

    // 选择客户
    console.log('  选择客户...');
    const customerSelect = await page.$('.el-dialog__body .el-select');
    if (customerSelect) {
    ...
    // 截图1: 表单填写完成
    const screenshot1 = path.join(SCREENSHOT_DIR, '6.1-form-filled.png');
    ...
    // 截图2: 保存结果
    const screenshot2 = path.join(SCREENSHOT_DIR, '6.1-success.png');
    await page.screenshot({ path: screenshot2, fullPage: true });
    console.log('  截图已保存:', screenshot2);

    // Step 6: 验证列表
    console.log('Step 6: 验证合同列表...');
    ...
    // 截图3: 列表显示
    const screenshot3 = path.join(SCREENSHOT_DIR, '6.1-list-data.png');
    ...
    const screenshotError = path.join(SCREENSHOT_DIR, '6.1-error.png');
...
test_6_1_create_contract().then(success => {
