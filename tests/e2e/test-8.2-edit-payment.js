...
async function test() {
  ...
  try {
    ...
    if (rows.length === 0) {
      console.log('  ⚠ 没有收款数据，跳过');
      return true;
    }
    console.log('Step 2: 当前收款记录数量:', rows.length);
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '8.2-form-filled.png'), fullPage: true });

    // 修改备注
    const remarkInput = await page.$('.el-dialog__body textarea');
    if (remarkInput) {
      await remarkInput.fill('收款备注_已修改_' + Date.now());
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '8.2-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 8.2 编辑收款单');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
