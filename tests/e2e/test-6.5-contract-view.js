...
async function test() {
  ...
  try {
    ...
    if (viewBtn) {
      await viewBtn.click();
      await page.waitForTimeout(2000);
      console.log('Step 4: 点击查看');
    } else {
      console.log('  ⚠ 未找到查看按钮');
    }

    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '6.5-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 6.5 查看合同详情');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
