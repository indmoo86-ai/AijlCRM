...
async function test() {
  ...
  try {
    ...
    if (rows.length > 0) {
      // 查找回访按钮
      const returnBtn = await page.$('.el-table__body-wrapper .el-table__row:first-child button:has-text("回访")');
      if (returnBtn) {
        await returnBtn.click();
        await page.waitForTimeout(2000);
        console.log('Step 4: 点击回访');
      } else {
        console.log('  ⚠ 未找到回访按钮');
      }
    }

    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '10.9-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 10.9 售后回访记录');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
