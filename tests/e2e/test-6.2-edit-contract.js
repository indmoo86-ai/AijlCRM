...
async function test() {
  ...
  try {
    ...
    if (rows.length === 0) {
      console.log('  ⚠ 没有合同数据，跳过');
      return true;
    }
    console.log('Step 2: 当前合同数量:', rows.length);
    ...
    if (remarkInput) {
      await remarkInput.fill('合同备注_已修改_' + Date.now());
      console.log('Step 4: 修改备注信息');
    }

    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '6.2-form-filled.png'), fullPage: true });
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '6.2-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 6.2 编辑合同');
