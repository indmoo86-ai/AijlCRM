...
async function test() {
  ...
  try {
    ...
    if (assignBtn) {
      ...
      await page.waitForTimeout(2000);
      console.log('Step 4: 点击分配');
    } else {
      console.log('  ⚠ 未找到分配按钮');
    }

    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '10.5-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 10.5 工单分配');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
