...
async function test() {
  ...
  try {
    ...
    console.log('Step 2: 当前客户数量:', rows.length);
    ...
    if (remarkInput) {
      await remarkInput.fill('测试备注_已修改');
      console.log('Step 4: 修改备注信息');
    }

    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '3.2-form-filled.png'), fullPage: true });
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '3.2-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 3.2 编辑客户');
