...
async function test() {
  ...
  try {
    ...
    if (rows.length === 0) {
      console.log('  ⚠ 没有发货单数据，跳过');
      return true;
    }
    console.log('Step 2: 当前发货单数量:', rows.length);
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '7.2-form-filled.png'), fullPage: true });

    // 修改备注
    const remarkInput = await page.$('.el-dialog__body textarea');
    if (remarkInput) {
      await remarkInput.fill('发货备注_已修改_' + Date.now());
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '7.2-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 7.2 编辑发货单');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
