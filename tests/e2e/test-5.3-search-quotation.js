...
async function test() {
  ...
  try {
    ...
    const searchInput = await page.$('input[placeholder*="报价单"]');
    if (searchInput) {
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '5.3-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 5.3 搜索报价单');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
