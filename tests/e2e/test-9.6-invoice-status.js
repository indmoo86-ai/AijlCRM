...
async function test() {
  ...
  try {
    ...
    console.log('Step 3: 当前发票数量:', rows.length);

    if (rows.length > 0) {
      // 尝试点击作废按钮
      const voidBtn = await page.$('.el-table__body-wrapper .el-table__row:first-child button:has-text("作废")');
      if (voidBtn) {
        await voidBtn.click();
        await page.waitForTimeout(1000);
        console.log('Step 4: 点击作废');
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '9.6-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 9.6 发票状态变更');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
