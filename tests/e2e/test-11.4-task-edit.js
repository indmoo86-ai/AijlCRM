  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      if (editBtn) {
        await editBtn.click();
        await page.waitForTimeout(2000);
        console.log('Step 4: 点击编辑');
      } else {
        console.log('  ⚠ 未找到编辑按钮');
      }
    }

    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '11.4-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 11.4 编辑任务');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
