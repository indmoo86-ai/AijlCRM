...
async function test() {
  ...
  try {
    ...
    if (rows.length > 0) {
      // 查找终止按钮
      const terminateBtn = await page.$('.el-table__body-wrapper .el-table__row:first-child button:has-text("终止")');
      if (terminateBtn) {
        await terminateBtn.click();
        await page.waitForTimeout(2000);
        console.log('Step 4: 点击终止');
        ...
        console.log('  ⚠ 未找到终止按钮（可能合同状态不允许）');
      }
    }

    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '6.8-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 6.8 合同终止');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
