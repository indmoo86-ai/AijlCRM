...
async function test() {
  ...
  try {
    ...
    if (rows.length === 0) {
      console.log('  ⚠ 没有发票数据，跳过');
      return true;
    }
    console.log('Step 2: 当前发票数量:', rows.length);
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '9.2-form-filled.png'), fullPage: true });

    await page.click('.el-dialog__footer button:has-text("确定")');
    await page.waitForTimeout(2000);

    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '9.2-success.png'), fullPage: true });

    console.log('========================================');
    console.log('场景: 9.2 编辑发票');
    console.log('状态: ✓ 通过');
    console.log('========================================');
    return true;
